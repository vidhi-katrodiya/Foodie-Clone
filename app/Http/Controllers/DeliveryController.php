...
class DeliveryController extends Controller {
    ...
    public function index(){
        $lang = Lang_core::all();
        return view("admin.delivery.default")->with("lang",$lang);
    }

    public function login(){
        $lang = Lang_core::all();
        return view("deliveryboy.login")->with("lang",$lang);
    }
    
    public function show_document_verification(){
        $lang = Lang_core::all();
        return view("admin.delivery.document_verification")->with("lang",$lang);
    }
    ...
    public function deliverydatatable(){
        ...
        return DataTables::of($item)
            ...
            ->editColumn('action', function ($item) {   
               $delete= url('admin/deleteboy',array('id'=>$item->id));
               
               if(Session::get("is_demo")=='1'){
                      $return = '<a onclick="edit_record(' . "'" . $item->id . "'" . ')" rel="tooltip" title="" class="m-b-10 m-l-5" data-original-title="Remove" style="margin-right:10px" data-toggle="modal" data-target="#editboy"><i class="fa fa-edit f-s-25"></i></a>
                            <a onclick="disablebtn()"class="m-b-10 m-l-5" ><i class="fa fa-trash f-s-25"></i></a>'; 
             
                 }else{
                      $return = '<a onclick="edit_record(' . "'" . $item->id . "'" . ')" rel="tooltip" title="" class="m-b-10 m-l-5" data-original-title="Remove" style="margin-right:10px" data-toggle="modal" data-target="#editboy"><i class="fa fa-edit f-s-25"></i></a>
                            <a onclick="delete_record(' . "'" . $delete . "'" . ')" rel="tooltip" title="" class="m-b-10 m-l-5" data-original-title="Remove"><i class="fa fa-trash f-s-25"></i></a>'; 
    ...
    public function documentverificationdatatable(){
          $item =Deliveryboy_Document::orderBy('id','DESC')->where("status",'0')->get();
        ...
        return DataTables::of($item)
            ...
            ->editColumn('name', function ($item) {
                return Delivery::find($item->deliveryboy_id)?Delivery::find($item->deliveryboy_id)->name:'';
            })
              ->editColumn('document_type', function ($item) {
                return $item->name;
            })
             ->editColumn('document', function ($item) {
                return asset('public/upload/images/Delivery/doc').'/'.$item->document;
            })
            ->editColumn('action', function ($item) {   
               $accept_url= url('admin/document_status',array('id'=>$item->id,'status'=>1));
               $reject_url= url('admin/document_status',array('id'=>$item->id,'status'=>2));
              
                 if(Session::get("is_demo")=='1'){
                      $return = '<a onclick="disablebtn()"  class="btn btn-sm btn-success" style="color:white !important">'.__('messages.accept').'</a>
                        <a onclick="disablebtn()" class="btn btn-sm btn-danger" style="margin-left:10px;color:white !important" >'.__('messages.reject').'</a>';
                 }else{
                      $return = '<a href="'.$accept_url.'" rel="tooltip" title="" class="btn btn-sm btn-success" data-original-title="Remove" style="color:white !important">'.__('messages.accept').'</a>
                        <a href="'.$reject_url.'" rel="tooltip" title="" class="btn btn-sm btn-danger" style="margin-left:10px;color:white !important" data-original-title="Remove">'.__('messages.reject').'</a>';
                 }
    ...
    public function document_status($id,$status){
         $getdoc = Deliveryboy_Document::find($id);
         if($getdoc){
             $getdoc->status = $status;
             $getdoc->save();
             if($status==1){
                 $msg = __('messages.Document Accept Successfully');
             }else{
                 $msg = __('messages.Document Reject Successfully');
             }
             $noti_key=Setting::find(1);
             $this->send_notification_android($noti_key->android_api_key,$getdoc->deliveryboy_id,$msg,"delivery_boyid");
             $this->send_notification_IOS($noti_key->iphone_api_key,$getdoc->deliveryboy_id,$msg,"delivery_boyid");
             Session::flash('message',$msg); 
             Session::flash('alert-class', 'alert-success');
             return redirect()->back();
         }else{
              Session::flash('message',__('messages.Document Not Found')); 
              Session::flash('alert-class', 'alert-danger');
              return redirect()->back();
         }
    ...
    public function add_delivery_boy(Request $request){
      ...
      $store->action=0;
      $store->save();
      Session::flash('message',__('messages.delivery_boy_add_success')); 
      Session::flash('alert-class', 'alert-success');
      return redirect("admin/deliveryboys");
      }
      else{
        Session::flash('message',__('messages.email_already_error')); 
      Session::flash('alert-class', 'alert-danger');
      return redirect("admin/deliveryboys");
      }
    ...
    public function update_delivery_boy(Request $request){
      ...
      $store->action=0;
      $store->save();
      Session::flash('message',__('messages.delivery_update_success')); 
      Session::flash('alert-class', 'alert-success');
      return redirect("admin/deliveryboys");
    }
      public function delete($id){
       ...
       $store->is_deleted='1';
       $store->save();
        Session::flash('message',__('messages.delivery_del_success')); 
         Session::flash('alert-class', 'alert-success');
         return redirect("admin/deliveryboys");
     }
      ...
      date_default_timezone_set($gettimezone);
         ...
         $update=Order::find($id);
              if($update){
                 ...
                 $this->send_notification_android($setting->android_api_key,$update->user_id,$msg,"user_id");
                 $this->send_notification_IOS($setting->iphone_api_key,$update->user_id,$msg,"user_id");
     ...
     public function ordercomplete($id){
      ...
      date_default_timezone_set($gettimezone);
        ...
        public function send_notification_android($key,$user_id,$msg,$field_type){
          $getuser=Token::where("type","android")->orwhere($field_type,$user_id)->get();
        if(count($getuser)!=0){               
               $reg_id = array();
   ...
   public function send_notification_IOS($key,$user_id,$msg,$field_type){
      $getuser=Token::where("type","Iphone")->orwhere($field_type,$user_id)->get();
         if(count($getuser)!=0){               
               $reg_id = array();
               foreach($getuser as $gt){
                   $reg_id[]=$gt->token;
