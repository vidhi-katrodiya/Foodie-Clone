"use strict"
    ...
    var totalrow=$("#totalrow_"+lang).val();    
    var newrow=parseInt(totalrow)+parseInt(1);
    var txt=' <div class="category-wrap" data-id="'+lang+"_"+newrow+'" id="mainattr_'+lang+"_"+newrow+'"><h3 class="uk-accordion-title uk-background-secondary uk-light uk-padding-small"><div class="uk-sortable-handle sort-categories uk-display-inline-block ti-layout-grid4-alt" ></div>'+$("#new_attribute").val()+'</h3><div class="uk-accordion-content categories-content " style="margin-top: 0px;padding:0px"><table class="table table-striped table-bordered"><tbody><tr><td><input type="text" name="attributeset['+newrow+']['+lang+'][set]" required class="form-control" ><table class="table table-striped table-bordered cmr1"><thead><tr><th>'+$("#attribute_msg").val()+'</th><th>'+$("#value_msg").val()+'</th><th></th></tr></thead><tbody id="morerow'+newrow+'"><tr id="attrrow_'+lang+newrow+'1"><td><input required class="form-control" type="text" name="attributeset['+newrow+']['+lang+'][label][]"></td><td><input class="form-control" type="text" required name="attributeset['+newrow+']['+lang+'][value][]"></td><td><button type="button" onclick="removeattrrow('+newrow+',1,\'' + lang + '\')" class="btn btn-danger"><i class="fa fa-trash f-s-25"></i></button></td></tr></tbody></table><button type="button" class="btn btn-primary fleft" onclick="addattrrow('+newrow+',\'' + lang + '\')"><i class="fa fa-plus"></i>Add New Row</button></td><td><button onclick="removerowmain('+newrow+',\'' + lang + '\')" class="btn btn-danger"><i class="fa fa-trash f-s-25"></i></button></td></tr></tbody></table></div></div>';
    $("#attributelist_"+lang).append(txt);
