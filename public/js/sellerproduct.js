...
function getsubcategory(val) {
            $('#subcategory').empty();
            $.ajax({
                url: $("#url_path").val()+"/seller/getsubcategory" + "/" + val,
                data: {},
...
function getbrand(val) {
            $('#brand').empty();
            $.ajax({
                url: $("#url_path").val()+"/seller/getbrandbyid" + "/" + val,
                data: {},
                success: function(data) {
 ...
 $(document).ready(function() {
          ...
          var strCopy = rel_pro.split(",");
           ...
           var example=$('#related_product').DataTable({
                processing: true,
                serverSide: true,
                ajax: $("#url_path").val()+'/seller/productlist'+'/'+related_cat+'/'+product_id,
                columns: [{
                    data: 'id',
                    name: 'id',
                }, {
            ...
            $.ajax( {
                         url: $("#url_path").val()+"/seller/checktotalproduct",
                         method:"get",
                         success: function( data ) {
                             if(data==1){
 ...
 $(document).ready(function() {
            $('#review_product').DataTable({
                processing: true,
                serverSide: true,
                ajax: $("#url_path").val()+'/seller/reviewdatatable'+"/"+$("#product_id").val(),
                columns: [{
                    data: 'id',
                    name: 'id'
                }, {
...
$(function() {
    ...
    function customRangenew(input) {
      ...
      function addglobaloption(){
          ...
          if(optionid!=""){
             $.ajax({
              url: $("#url_path").val()+"/seller/getoptionvalues" + "/" +optionid,
              data: {},
              success: function(data) {
    ...
    $(document).ready(function() {
         ...
         if(product_id!=0){
            ...
            $('#review_product').DataTable({
                processing: true,
                serverSide: true,
                ajax: $("#url_path").val()+'/seller/reviewdatatable'+"/"+"abc",
                columns: [{
                    data: 'id',
                    name: 'id'
                }, {
