...
$(document).ready(function () {
    $('#ordercompletecustomerTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: $("#url_path").val() + '/admin/completepaymentdatatable',
        columns: [{
            data: 'id',
            name: 'id'
        }, {    
 ...
 $(document).ready(function() {
    ...
    $(document).ready(function() {
        ...
        $('#documentverificationtable').DataTable({
            processing: true,
            serverSide: true,
            ajax: $("#url_path").val()+'/admin/documentverificationdatatable',
            columns: [{
                data: 'id',
                name: 'id'
            }, {
   ...
   $("#order_id").val(data);
     ...
     $(document).ready(function() {
        $('#Citydatatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: $("#url_path").val()+'/admin/Citydatatable',
            columns: [{
                data: 'id',
                name: 'id'
            }, {
...
$(document).ready(function () {
    $('#boytb').DataTable({
        processing: true,
        serverSide: true,
        ajax: $("#url_path").val()+'/admin/deliverydatatable',
        columns: [
            {data: 'id'    , name: 'id'},
            {data: 'name'  , name: 'name'},
...
function edit_record(id){
     $.ajax( {
         url: $("#url_path").val()+"/admin/editdeliveryboys"+"/"+id,
         data: { },
     ...
     $(document).ready(function() {
        $('#ordercurrentpaymentTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: $("#url_path").val()+'/admin/ordercurrentpaymentTable',
            columns: [{
                data: 'id',
                name: 'id'
            }, {
  ...
  function delete_record(url) {
    ...
    function editcity(id) {
        $.ajax({
            url: $("#url_path").val()+"/admin/getcitybyid" + "/" + id,
            data: {},
            success: function(data) {
                $('#id').val(id);
                var str = JSON.parse(data);
                for (var i=0 ; i< str.language.length;i++) {
                       if($("#name_"+str.language[i].code).length){
                            $("#name_"+str.language[i].code).val("");
                           var txt = "name"+"_"+str.language[i].code;
                           var keysls= Object.keys(str.data);
                           var valuels = Object.values(str.data);
                           for (var j=0; j <keysls.length;j++) {
                               if(keysls[j]==txt){
                                    $("#name_"+str.language[i].code).val(valuels[j]);
                               }
                           }
 ...
 function editcategory(id) {
        $.ajax({
            ...
            success: function(data) {
                 ...
                 console.log(str);
                    $("#id").val(str.data.id);
                    $("#delivery_charges").val(str.data.delivery_charges);
                    $("#edit_img").attr("src",$("#url_path").val()+'/public/upload/category'+"/"+str.data.image);
                 //  $('#real_image').val(str.data.cat_icon);
                   for (var i=0 ; i< str.language.length;i++) {
                       if($("#name_"+str.language[i].code).length){
                            $("#name_"+str.language[i].code).val("");
                           var txt = "name"+"_"+str.language[i].code;
...
$(document).ready(function(){
    $("#addcoupon").on("submit", function(event){
        ...
        $.post($("#url_path").val()+"/admin/savecoupon", formValues, function(data){
         if (data != "Code") {
                                $("#coupon_id").val(data);
...
function chnagevalue(val) {
         ...
         function closeedit() {
            ...
            $('#transalteTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: $("#url_path").val()+'/admin/translationdatatable'+'/'+$("#code").val(),
                columns: [{
                    data: 'id',
                    name: 'id'
                }, {
...
$(document).ready(function() {
  ...
  $(document).ready(function() {
    ...
    function removerow(val,lang){
      if(val!=1){
          
          $("#row_"+lang+"_"+val).remove();
      }
    ...
    function addoptionrow(lang) {
        var lastrow=$("#totalrow_"+lang).val();
        var newrow=parseInt(lastrow)+1;
        var txt = '<tr id="row_'+lang+"_"+newrow+'"><td><i class="ti-layout-grid4-alt"></i></td><td data-id="'+newrow+'"><input type="text" required id="label_'+lang+'_'+newrow+'" name="label_'+lang+'[]" placeholder="" class="form-control"></td><td><input type="text"  id="price_'+lang+"_"+newrow+'" name="price_'+lang+'[]" placeholder="" class="form-control"></td><td><button onclick="removerow('+newrow+',\'' + lang + '\')" type="button" class="btn btn-danger"><i class="fa fa-trash f-s-25"></i></button></td></tr>';
        $('#lstable_'+lang).append(txt);
        $("#totalrow_"+lang).val(newrow);
    }
       ...
       $('#productdataTable').DataTable({
          ...
          columns: [
           ...
           columnDefs: [
            { targets: 1,
              render: function(data) {
                    return '<img src="'+data+'" style="height:60px; width:82px;" >';
              }
            }   
        ],
         order:[[0,"DESC"]]
 ...
 $(document).ready(function () {
       $('#unapproveproductdataTable').DataTable({
          processing: true,
          serverSide: true,
          ajax: $("#url_path").val()+'/admin/unapproveproductdataTable',
          columns: [
            {data: 'id'    , name: 'id'},
...
$(document).ready(function(){
    $("#updatesetting").on("submit", function(event){
        event.preventDefault();
 
        var formValues= $(this).serialize();
 
        $.post($("#url_path").val()+"/admin/savegeneralsetting", formValues, function(data){
            $("#general").removeClass('in show active');
                            $('a[href="#general"]').removeClass('active');
                            $('a[href="#login"]').addClass('active');
                            $("#login").addClass('in show active');
 ...
 $(document).ready(function () {
       ...
       $.ajax( {
                     ...
                     success: function( data ) {
                        var str=JSON.parse(data);
                         $("#edit_id").val(id);
                         for (var i=0 ; i< str.language.length;i++) {
                       if($("#topicname_"+str.language[i].code).length){
                            $("#topicname_"+str.language[i].code).val("");
                           var txt = "topicname"+"_"+str.language[i].code;
                           var keysls= Object.keys(str.data);
                           var valuels = Object.values(str.data);
                           for (var j=0; j <keysls.length;j++) {
                               if(keysls[j]==txt){
                                    $("#topicname_"+str.language[i].code).val(valuels[j]);
                               }
                           }
   ...
   function editques(id){
       $.ajax( {
                     ...
                     success: function( data ) {
                         ...
                         $("#edit_id").val(id);
                             for (var i=0 ; i< str.language.length;i++) {
                       if($("#ques_"+str.language[i].code).length){
                            $("#ques_"+str.language[i].code).val("");
                           var txt = "ques"+"_"+str.language[i].code;
                           var keysls= Object.keys(str.data);
                           var valuels = Object.values(str.data);
                           for (var j=0; j <keysls.length;j++) {
                               if(keysls[j]==txt){
                                    $("#ques_"+str.language[i].code).val(valuels[j]);
                               }
                           }
                       ...
                       for (var i=0 ; i< str.language.length;i++) {
                       if($("#ans_"+str.language[i].code).length){
                            $("#ans_"+str.language[i].code).val("");
                           var txt = "ans"+"_"+str.language[i].code;
                           var keysls= Object.keys(str.data);
                           var valuels = Object.values(str.data);
                           for (var j=0; j <keysls.length;j++) {
                               if(keysls[j]==txt){
                                    $("#ans_"+str.language[i].code).val(valuels[j]);
                               }
                           }
     ...
     function checkbothpwd(val){
      ...
      $(document).ready(function () {
       ...
       $('#sellerTable').DataTable({
          processing: true,
          serverSide: true,
          ajax: $("#url_path").val()+'/admin/sellerTable',
          columns: [
            {data: 'id'    , name: 'id'},
            {data: 'name'  , name: 'name'},
            {data: 'email'  , name: 'email'},
   ...
   function editseller(id){
         $.ajax( {
                     url: $("#url_path").val()+"/admin/editseller"+"/"+id,
                     data: { },
                     success: function( str ) {
                       var data=JSON.parse(str);
                       console.log(data);
                         ...
                         $("#edit_access_cat").html(data.options);
                     }
                 });
   }
...
function play_sound() {
    ...
    $('#language_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: $("#url_path").val()+'/admin/language_datatable',
        columns: [
            {data: 'id'    , name: 'id'},
            {data: 'name'    , name: 'name'},
            {data: 'code'  , name: 'code'},
 ...
 function changenotificationtype(val){
        if(val==0){
            $("#normalnotification").css("display","block");
            $("#productnotification").css("display","none");
            $("#product_id").attr("required",false);
            $("#msg").attr("required",true);
        }

        if(val==1){
            $("#normalnotification").css("display","none");
            $("#productnotification").css("display","block");
            $("#product_id").attr("required",true);
 ...
 $(document).ready(function () {
    $('#Pending_payment_Table').DataTable({
        processing: true,
        serverSide: true,
        ajax: $("#url_path").val() + '/admin/pendingpaymentdatatable',
        columns: [{
                data: 'id',
                name: 'id'
            }, {
